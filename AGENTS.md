...
`llm-auto-review-latest` reads the latest consolidated LLM auto-review queue and renders an owner decision view across all queued chains and skipped-existing chain summaries, including player-value checks, owner questions, blocking issues, and intake commands. It is read-only and must not call an LLM provider, approve content edits, or mutate repository state.
