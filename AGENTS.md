...
`llm-issue-lifecycle` records one owner-approved lifecycle step for an existing planned manifest from a GitHub owner handoff issue comment. It accepts only `/review-run <run_id> <owner note>`, `/brief-run <run_id> <owner note>`, or `/patch-plan-run <run_id> <owner note>` posted on `LLM Owner Digest: Action Needed`, requires the manifest to be in the exact previous status, and may only mutate automation manifests plus brief/patch-plan reports. It must not approve public copy, patch specs, backlog edits, public content, PRs, or production state.
