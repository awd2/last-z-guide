...
`llm-owner-issue` creates or updates one GitHub owner handoff issue from `llm-owner-digest.json` only when the digest state is `owner_review_needed`, `ready_for_intake`, or `blocked_or_failed`. Non-actionable states are no-op. It is a notification layer only: it must not approve content edits, mutate backlog/manifests, edit public content, open PRs, or deploy.
