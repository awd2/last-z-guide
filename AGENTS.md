...
python3 automation/workers/llm_issue_manifest.py --comment-body "/dry-run-manifest <topic_id> <owner note>" --comment-author <github_login> --author-association OWNER --json
...
The `openai` LLM adapter provider is live but still no-write and fail-closed. It requires `OPENAI_API_KEY`, uses `OPENAI_MODEL` when set, defaults to `gpt-5.4-mini`, and returns only validated plain-ASCII JSON artifacts. The only role-specific exception is Editor `response_json.exact_replacements`, which may pass through adapter ASCII validation so the Editor wrapper can deterministically drop unsafe snippets with warnings before Reviewer. It must not edit content, backlog, manifests, or production state.
...
`external-search-collect` is the no-write external search layer. With `--provider openai`, it uses the OpenAI Responses API `web_search` tool over approved `source_query_tasks`, constrained by source-domain filters when available, and emits proposal-shaped discovery leads for LLM Scout. It must normalize raw search clusters/targets against `content_index.json`, attach deterministic mapping/score metadata, and downgrade generic or weakly mapped results to monitor-only. Search evidence is discovery context only: it must not prove public claims, approve public copy, mutate backlog/manifests, edit content, open PRs, or deploy.
...
`llm-auto-review-queue` runs candidate refresh, scores candidate topics, and automatically runs the top no-write Editor/Reviewer chains into one consolidated owner-review queue. It may accept External Scout proposal artifacts through `--external-proposals`. It skips only current completed chain summaries unless `--include-existing` is supplied; stale summaries with missing or older `worker_chain_contract_version` are rerun automatically so worker contract upgrades can refresh old chain artifacts. Topics with recorded `llm-topic-decision-*` states of `monitor`, `rejected`, or `approved_for_chain` are moved to `resolved_by_decision` instead of queueing or skipped-existing, so already reviewed topics do not create daily owner-work noise. If candidate refresh fail-closes only because live Scout selected a topic with a recorded owner decision, the queue should recover into a green `current` report with `resolved_refresh_errors` instead of failing the scheduled workflow. Queue `next_actions` are state-aware: resolved-only queues say no owner action is needed, failed queues point to stage errors, and queued items point to the intake lifecycle. Per-topic LLM stage failures are recorded as `completed_with_failures` review artifacts but do not make the command exit non-zero, so scheduled workflows can still upload and commit no-write reports. It must not approve public copy, mutate backlog, create manifests, edit content, open PRs, or deploy.
...
`llm-editor` is the second live LLM worker wrapper. It creates a planning brief from one selected LLM Scout opportunity and deterministic Editor context. For `update_existing` opportunities on existing HTML targets, it should prefer narrow draft `exact_replacements` when `current_page_snapshot.source_snippets` contains enough current source text to copy `exact_old` literally. These candidates must preserve existing wrappers/template classes unless a template change was explicitly requested, and every item must set `owner_approval_required: true`. No-op, non-ASCII, nonliteral, missing-approval, wrong-target, or otherwise unsafe exact candidates are dropped with warnings before Reviewer; the planning brief may continue with an empty `exact_replacements` list. This does not approve copy, create Patch Specs, edit files, or bypass the proposal/approval/apply lifecycle. It must not write final public page copy, patch specs, content edits, backlog entries, manifests, PRs, or production state.
...
GitHub workflow `.github/workflows/llm-owner-decision.yml` records owner handoff comments on the `LLM Owner Digest: Action Needed` issue. It accepts only `/monitor`, `/reject`, `/approve-chain`, `/approve-intake`, `/approve-run-plan`, `/dry-run-manifest`, `/approve-manifest`, `/review-run`, `/brief-run`, `/patch-plan-run`, `/propose-run`, `/approve-proposal`, and `/preview-apply` comments from `OWNER`, `MEMBER`, or `COLLABORATOR` author associations, uploads no-write summary artifacts, replies to the same issue with the result, refreshes the issue body with the latest Active Run Lifecycle, and may commit only owner handoff artifacts, automation manifest files under `automation/manifests/`, and generated brief/patch/proposal/apply-preview reports. For `/approve-chain`, it runs the no-write `llm-worker-chain --from-decision` path in the same workflow job because commits made with `GITHUB_TOKEN` do not reliably trigger downstream push workflows, then persists the no-write chain summary for later `/approve-intake`. For `/approve-intake`, it records intake-only approval from a matching completed chain summary. For `/approve-run-plan`, it creates a no-write run-plan proposal from a matching approved intake artifact. For `/dry-run-manifest`, it validates the planned manifest path without writing one. For `/approve-manifest`, it may create only a `planned` run manifest. For `/review-run`, `/brief-run`, `/patch-plan-run`, and `/propose-run`, it advances an existing manifest only through the deterministic proposal lifecycle: `planned -> reviewed -> draft_brief_ready -> patch_plan_ready -> proposal_ready`. For `/approve-proposal`, it records owner approval for already rendered proposal specs and may move a manifest to `approved_for_apply`, but it still does not apply content. For `/preview-apply`, it renders no-write apply-preview artifacts and may move a manifest to `apply_preview_ready`, but it still does not apply content. It still must not edit public content, backlog, PRs, or production state.
...
`llm-issue-intake` records one intake-only owner approval from a GitHub owner handoff issue comment. It accepts only `/approve-intake <topic_id> <owner note>` posted on `LLM Owner Digest: Action Needed`, requires a matching completed `llm-worker-chain-<topic_id>.json` summary in `automation/reports/llm-owner-decision-chains/`, `automation/reports/llm-auto-review-queue/`, or `automation/reports/`, and writes only `llm-intake-<topic_id>` artifacts. It must not approve public copy, patch specs, backlog edits, manifests, public content, PRs, or production state.

`llm-issue-run-plan` records one run-plan-only owner approval from a GitHub owner handoff issue comment. It accepts only `/approve-run-plan <topic_id> <owner note>` posted on `LLM Owner Digest: Action Needed`, requires a matching approved `llm-intake-<topic_id>.json` or `worker-intake-<topic_id>.json` artifact, and writes only `llm-worker-run-plan-<topic_id>` artifacts. It must not approve public copy, patch specs, backlog edits, manifests, public content, PRs, or production state.

`llm-issue-manifest` records one owner manifest action from a GitHub owner handoff issue comment. It accepts only `/dry-run-manifest <topic_id> <owner note>` or `/approve-manifest <topic_id> <owner note>` posted on `LLM Owner Digest: Action Needed`, requires a matching `run_plan_ready` `llm-worker-run-plan-<topic_id>.json` or `worker-run-plan-<topic_id>.json`, and may create only a `planned` run manifest for `/approve-manifest`. It must not approve public copy, patch specs, backlog edits, public content, PRs, or production state.

`llm-issue-lifecycle` records one owner-approved lifecycle step for an existing planned manifest from a GitHub owner handoff issue comment. It accepts only `/review-run <run_id> <owner note>`, `/brief-run <run_id> <owner note>`, `/patch-plan-run <run_id> <owner note>`, or `/propose-run <run_id> <owner note>` posted on `LLM Owner Digest: Action Needed`, requires the manifest to be in the exact previous status, and may only mutate automation manifests plus brief/patch/proposal reports. It must not approve public copy, patch specs, backlog edits, public content, PRs, or production state.
