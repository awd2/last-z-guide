...
GitHub workflow `.github/workflows/llm-owner-decision.yml` records owner handoff comments on the `LLM Owner Digest: Action Needed` issue. It accepts only `/monitor`, `/reject`, `/approve-chain`, and `/approve-intake` comments from `OWNER`, `MEMBER`, or `COLLABORATOR` author associations, uploads no-write summary artifacts, replies to the same issue with the result, and may commit only `automation/reports/llm-topic-decision-<topic_id>.json` / `.md`, `automation/reports/llm-owner-decision-chains/llm-worker-chain-<topic_id>.json` / `.md`, and `automation/reports/llm-intake-<topic_id>.json` / `.md` owner handoff artifacts. For `/approve-chain`, it runs the no-write `llm-worker-chain --from-decision` path in the same workflow job because commits made with `GITHUB_TOKEN` do not reliably trigger downstream push workflows, then persists the no-write chain summary for later `/approve-intake`. For `/approve-intake`, it records intake-only approval from a matching completed chain summary. It must not edit content, backlog, manifests, PRs, or production state.
