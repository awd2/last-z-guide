...
def cmd_llm_issue_decision(
    ...
    command = [
        sys.executable,
        str(AUTOMATION_DIR / "workers" / "llm_issue_decision.py"),
        "--comment-body",
        comment_body,
    ]
    if comment_author:
...
def cmd_llm_issue_run_plan(
    ...
    command = [
        sys.executable,
        str(AUTOMATION_DIR / "workers" / "llm_issue_run_plan.py"),
        "--comment-body",
        comment_body,
    ]
    if comment_author:
    ...
    if as_json:
        command.append("--json")
        return subprocess.run(command, cwd=ROOT).returncode
    print("\n== LLM Issue Run Plan ==", flush=True)
    return subprocess.run(command, cwd=ROOT).returncode


def cmd_llm_issue_manifest(
    comment_body: str,
    comment_author: str | None,
    author_association: str | None,
    issue_title: str | None,
    ...
    command = [
        sys.executable,
        str(AUTOMATION_DIR / "workers" / "llm_issue_manifest.py"),
        "--comment-body",
        comment_body,
    ]
    if comment_author:
    ...
    if as_json:
        command.append("--json")
        return subprocess.run(command, cwd=ROOT).returncode
    print("\n== LLM Issue Manifest ==", flush=True)
    return subprocess.run(command, cwd=ROOT).returncode


def cmd_llm_issue_lifecycle(
    comment_body: str,
    comment_author: str | None,
    author_association: str | None,
    issue_title: str | None,
    ...
    command = [
        sys.executable,
        str(AUTOMATION_DIR / "workers" / "llm_issue_lifecycle.py"),
        "--comment-body",
        comment_body,
    ]
    if comment_author:
    ...
    if as_json:
        command.append("--json")
        return subprocess.run(command, cwd=ROOT).returncode
    print("\n== LLM Issue Lifecycle ==", flush=True)
    return subprocess.run(command, cwd=ROOT).returncode
...
def cmd_llm_issue_apply_preview(
    comment_body: str,
    comment_author: str | None,
    author_association: str | None,
    issue_title: str | None,
    ...
    command = [
        sys.executable,
        str(AUTOMATION_DIR / "workers" / "llm_issue_apply_preview.py"),
        "--comment-body",
        comment_body,
    ]
    if comment_author:
    ...
    if as_json:
        command.append("--json")
        return subprocess.run(command, cwd=ROOT).returncode
    print("\n== LLM Issue Apply Preview ==", flush=True)
    return subprocess.run(command, cwd=ROOT).returncode
...
def build_parser() -> argparse.ArgumentParser:
    ...
    llm_issue_decision_parser = subparsers.add_parser(
        "llm-issue-decision",
        help="Record an owner topic decision from a GitHub owner issue comment command.",
    )
    llm_issue_decision_parser.add_argument("--comment-body", required=True, help="Raw GitHub issue comment body.")
    llm_issue_decision_parser.add_argument("--comment-author", help="GitHub login for the comment author.")
    llm_issue_decision_parser.add_argument("--author-association", help="GitHub author association, such as OWNER or COLLABORATOR.")
    llm_issue_decision_parser.add_argument("--issue-title", help="GitHub issue title. Must match the owner digest handoff title.")
    llm_issue_decision_parser.add_argument("--discovery", help="Path to the topic discovery artifact.")
    llm_issue_decision_parser.add_argument("--output-dir", help="Directory for llm-topic-decision artifacts.")
    llm_issue_decision_parser.add_argument("--summary-output", help="Optional JSON summary output path.")
    ...
    llm_issue_run_plan_parser = subparsers.add_parser(
        "llm-issue-run-plan",
        help="Record run-plan-only owner approval from a GitHub owner issue comment command.",
    )
    llm_issue_run_plan_parser.add_argument("--comment-body", required=True, help="Raw GitHub issue comment body.")
    llm_issue_run_plan_parser.add_argument("--comment-author", help="GitHub login for the comment author.")
    llm_issue_run_plan_parser.add_argument("--author-association", help="GitHub author association, such as OWNER or COLLABORATOR.")
    llm_issue_run_plan_parser.add_argument("--issue-title", help="GitHub issue title. Must match the owner digest handoff title.")
    llm_issue_run_plan_parser.add_argument("--reports-dir", help="Directory to search for intake artifacts.")
    llm_issue_run_plan_parser.add_argument("--intake", help="Optional explicit llm-intake-<topic_id>.json path.")
    llm_issue_run_plan_parser.add_argument("--output-dir", help="Directory for run-plan artifacts.")
    llm_issue_run_plan_parser.add_argument("--summary-output", help="Optional JSON summary output path.")
    llm_issue_run_plan_parser.add_argument("--markdown-output", help="Optional markdown summary output path.")
    llm_issue_run_plan_parser.add_argument("--json", action="store_true", help="Print the issue run-plan summary as JSON.")

    llm_issue_manifest_parser = subparsers.add_parser(
        "llm-issue-manifest",
        help="Record dry-run or creation approval for a planned manifest from a GitHub owner issue comment command.",
    )
    llm_issue_manifest_parser.add_argument("--comment-body", required=True, help="Raw GitHub issue comment body.")
    llm_issue_manifest_parser.add_argument("--comment-author", help="GitHub login for the comment author.")
    llm_issue_manifest_parser.add_argument("--author-association", help="GitHub author association, such as OWNER or COLLABORATOR.")
    llm_issue_manifest_parser.add_argument("--issue-title", help="GitHub issue title. Must match the owner digest handoff title.")
    llm_issue_manifest_parser.add_argument("--reports-dir", help="Directory to search for run-plan artifacts.")
    llm_issue_manifest_parser.add_argument("--run-plan", help="Optional explicit llm-worker-run-plan-<topic_id>.json path.")
    llm_issue_manifest_parser.add_argument("--manifest-dir", help="Directory where planned manifests are written.")
    llm_issue_manifest_parser.add_argument("--summary-output", help="Optional JSON summary output path.")
    llm_issue_manifest_parser.add_argument("--markdown-output", help="Optional markdown summary output path.")
    llm_issue_manifest_parser.add_argument("--json", action="store_true", help="Print the issue manifest summary as JSON.")

    llm_issue_lifecycle_parser = subparsers.add_parser(
        "llm-issue-lifecycle",
        help="Run a planned manifest lifecycle step from a GitHub owner issue comment command.",
    )
    llm_issue_lifecycle_parser.add_argument("--comment-body", required=True, help="Raw GitHub issue comment body.")
    llm_issue_lifecycle_parser.add_argument("--comment-author", help="GitHub login for the comment author.")
    llm_issue_lifecycle_parser.add_argument("--author-association", help="GitHub author association, such as OWNER or COLLABORATOR.")
    llm_issue_lifecycle_parser.add_argument("--issue-title", help="GitHub issue title. Must match the owner digest handoff title.")
    llm_issue_lifecycle_parser.add_argument("--manifest-dir", help="Directory where planned manifests are stored.")
    llm_issue_lifecycle_parser.add_argument("--manifest", help="Optional explicit run manifest path.")
    llm_issue_lifecycle_parser.add_argument("--output-dir", help="Directory for brief or patch-plan artifacts.")
    llm_issue_lifecycle_parser.add_argument("--summary-output", help="Optional JSON summary output path.")
    ...
    llm_issue_apply_preview_parser = subparsers.add_parser(
        "llm-issue-apply-preview",
        help="Render no-write apply preview from a GitHub owner issue comment command.",
    )
    llm_issue_apply_preview_parser.add_argument("--comment-body", required=True, help="Raw GitHub issue comment body.")
    llm_issue_apply_preview_parser.add_argument("--comment-author", help="GitHub login for the comment author.")
    llm_issue_apply_preview_parser.add_argument("--author-association", help="GitHub author association, such as OWNER or COLLABORATOR.")
    llm_issue_apply_preview_parser.add_argument("--issue-title", help="GitHub issue title. Must match the owner digest handoff title.")
    llm_issue_apply_preview_parser.add_argument("--manifest-dir", help="Directory where run manifests are stored.")
    llm_issue_apply_preview_parser.add_argument("--manifest", help="Optional explicit run manifest path.")
    llm_issue_apply_preview_parser.add_argument("--output-dir", help="Directory for apply preview artifacts.")
    llm_issue_apply_preview_parser.add_argument("--summary-output", help="Optional JSON summary output path.")
    llm_issue_apply_preview_parser.add_argument("--markdown-output", help="Optional markdown summary output path.")
    llm_issue_apply_preview_parser.add_argument("--json", action="store_true", help="Print the apply preview summary as JSON.")
...
def main() -> int:
    ...
    if args.command == "llm-issue-decision":
        return cmd_llm_issue_decision(
            args.comment_body,
            args.comment_author,
    ...
    if args.command == "llm-issue-run-plan":
        return cmd_llm_issue_run_plan(
            args.comment_body,
            args.comment_author,
            args.author_association,
            args.issue_title,
    ...
    if args.command == "llm-issue-manifest":
        return cmd_llm_issue_manifest(
            args.comment_body,
            args.comment_author,
            args.author_association,
            args.issue_title,
    ...
    if args.command == "llm-issue-apply-preview":
        return cmd_llm_issue_apply_preview(
            args.comment_body,
            args.comment_author,
            args.author_association,
            args.issue_title,
