# Proposed Edits: 2026-05-01-research-atlas-home-promotion
...
Desired after:
Source adds a crawlable, user-visible bridge to `index.html` in the relevant section or related-guides block.
...
Desired after:
JSON source adds or strengthens a route back to `index.html` through `related_guides` or `next_steps`, then regenerates the HTML output.
...
Before:
title: Last Z Guides (2026) — Research, Events, HQ, Heroes, and F2P Strategy | h1: Last Z Guides — Research, Events, HQ, Heroes, and F2P Strategy | meta: Last Z guides for research order, HQ upgrades, events, heroes, formations, resource planning, and F2P progression. Start with the strongest pages for your current goal.

Desired after:
Opening answer matches the page role for index.html, gives the exact setup answer quickly, and avoids drifting into a broader hub page.
...
Desired after:
Title, H1, and meta description use the same exact intent: index.html.
...
Desired after:
Card or route blocks stay scoped to the target page role and help users choose the correct next page.
