...
`llm-issue-run-plan` is the GitHub issue-comment bridge from approved intake to
no-write run-plan proposal. It accepts only
`/approve-run-plan <topic_id> <owner note>` on the
`LLM Owner Digest: Action Needed` issue, requires a matching approved
`llm-intake-<topic_id>.json` or `worker-intake-<topic_id>.json`, and writes only
`automation/reports/llm-worker-run-plan-<topic_id>.json` and `.md`. This
approval is run-plan-only and must not approve public copy, mutate
backlog/manifests/content, create PRs, or deploy.

`llm-issue-manifest` is the GitHub issue-comment bridge from no-write run-plan
proposal to planned manifest. It accepts only
`/dry-run-manifest <topic_id> <owner note>` or
`/approve-manifest <topic_id> <owner note>` on the
`LLM Owner Digest: Action Needed` issue, requires a matching `run_plan_ready`
`llm-worker-run-plan-<topic_id>.json` or `worker-run-plan-<topic_id>.json`, and
may create only a `planned` manifest for `/approve-manifest`. It must not
approve public copy, mutate backlog/content, create PRs, or deploy.
...
`llm-issue-proposal-approval` is the GitHub issue-comment bridge from rendered
proposal specs to owner-approved proposal specs. It accepts only
`/approve-proposal <run_id> <owner note>` on the
`LLM Owner Digest: Action Needed` issue, requires a matching manifest with
rendered proposal specs, and may move the manifest to `approved_for_apply` while
refreshing `.proposed.md` and `.exact-proposals.*` reports. It allows the next
...
owner involvement to reviewing ready queue packages. Topics with recorded
