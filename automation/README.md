...
python3 automation/pipeline.py pre-apply-review 2026-04-22-research-cluster-nav
...
- refresh failures caused only by live Scout selecting a topic with a recorded owner decision recover into a green `current` report with `resolved_refresh_errors`
- `next_actions` are state-aware: resolved-only queues say no owner action is needed, failed queues point to stage errors, and queued items point to the intake lifecycle
...
- supported commands: `/monitor`, `/reject`, `/approve-chain`, `/approve-intake`, `/approve-run-plan`, `/dry-run-manifest`, `/approve-manifest`, `/review-run`, `/brief-run`, `/patch-plan-run`, `/propose-run`, `/approve-proposal`, and `/preview-apply`
...
- for `/preview-apply`, it renders no-write apply-preview artifacts and can move the manifest to `apply_preview_ready`, but it still does not apply public content
...
- `python3 automation/pipeline.py llm-owner-issue --json` -> create/update one GitHub owner handoff issue when the latest digest is actionable or active run lifecycle states exist, or close the previous handoff issue when both are non-actionable
...
- after `/preview-apply`, the Active Run Lifecycle section should show `apply_preview_ready`, no GitHub issue command, and a local-only final apply review handoff for `pre-apply-review`, `apply-approved`, strict manifest QA, and closeout
...
- `python3 automation/pipeline.py llm-issue-run-plan --comment-body "/approve-run-plan <topic_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> parse one owner handoff issue comment and create a no-write run-plan proposal from approved intake
- supported command is `/approve-run-plan`
- the command requires a topic id, a real owner note, an accepted author association, and a matching approved `llm-intake-<topic_id>.json` or `worker-intake-<topic_id>.json` artifact
...
- `python3 automation/pipeline.py llm-issue-lifecycle --comment-body "/review-run <run_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> run deterministic review on an existing `planned` manifest
- `python3 automation/pipeline.py llm-issue-lifecycle --comment-body "/brief-run <run_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> create a brief artifact from an existing `reviewed` manifest
- `python3 automation/pipeline.py llm-issue-lifecycle --comment-body "/patch-plan-run <run_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> create a proposal-only patch plan from an existing `draft_brief_ready` manifest
- `python3 automation/pipeline.py llm-issue-lifecycle --comment-body "/propose-run <run_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> render owner-review proposals from an existing `patch_plan_ready` manifest
...
- `python3 automation/pipeline.py llm-issue-apply-preview --comment-body "/preview-apply <run_id> <owner note>" --comment-author <github_login> --author-association OWNER --json` -> render a no-write apply preview from an existing `approved_for_apply` manifest
- supported command is `/preview-apply`
...
- once the manifest is `apply_preview_ready`, the owner issue should stop offering GitHub-side lifecycle commands for that run and instead show local-only `pre-apply-review`, apply, QA, and closeout commands
...
- `apply_preview_ready` -> `pre-apply-review`
...
`propose`, explicit owner `approval`, `apply-preview`, `pre-apply-review`, and `apply-approved`.
...
body, counts them separately, and does not mutate the manifest, record approval,
edit content, commit, push, or deploy.
...
`pre-apply-review` renders a local-only final review after `apply-preview`:
...
- `automation/reports/<run_id>.pre-apply-review.json`

It does not edit site content, does not mutate manifests, does not run
`apply-approved`, and does not create PRs or deploy. It validates approved specs
against deterministic apply support, checks exact source matches where possible,
and prints the local commands to run only after final owner approval.
